...
import com.youzan.wagon.filter.processor.mq.RocketmqService;
...
public class MQProcessor implements EntryProcessor {
    ...
    public MQProcessor() {
        if ("rocketmq".equals(PropertiesManager.get(WagonConstants.PRO_MQ_TYPE))) {
            this.mqService = new RocketmqService();
        } else {
            this.mqService = new NSQService(PropertiesManager.getProperties());
        }
    }
        ...
        if (entries != null && !entries.isEmpty()) {
            for (Entry entry : entries) {
                try {
                    ...
                    LOG_ERROR.error("DefaultEntryProcessor.process goes wrong at:{}", ExceptionUtils.getFullStackTrace(e));
                }
            }
        }
