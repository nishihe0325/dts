...
public interface CanalInstanceDao {
    ...
    String COMMON_COLMUNS = "service_id, service_name, instance_name, slave_id, db_host, db_host_name, db_port, db_username, db_password, binlog_file, binlog_offset, binlog_exe_time, service_state, create_time, modify_time";

    // ==================== insert ====================
    @Insert("INSERT INTO canal_instance (" + COMMON_COLMUNS + ") VALUES (#{serviceId}, #{serviceName}, #{instanceName}, #{slaveId}, #{dbHost}, #{dbHostName}, #{dbPort}, #{dbUsername}, #{dbPassword}, #{binlogFile}, #{binlogOffset}, #{binlogExeTime}, #{serviceState}, now(), now())")
    ...
    @Update("UPDATE canal_instance SET service_state=#{serviceState}, modify_time = now() WHERE id=#{id}")
    boolean updateStateById(CanalInstance canalInstance);
