...
public class CanalInstanceManageController {
    ...
    public String instanceManageByPost(Model model, CanalService param) {
        ...
        try {
            ...
            model.addAttribute(WagonConstants.MODEL_CUR_TIME, new Date().getTime());
        } catch (Throwable e) {
    ...
    @RequestMapping(value = WagonConstants.URL_UPDATE_CANAL_INSTANCE_STATE, method = RequestMethod.GET)
    public String updateCanalInstanceState(Model model, CanalInstance param) {
        String errMsg = null;
        param = param != null ? param : new CanalInstance();

        try {
            canalInstanceService.updateStateById(param);
        } catch (Throwable e) {
            LOG_ERROR.error("stopCanalInstance failed: {}", ExceptionUtils.getFullStackTrace(e));
            errMsg = "停止失败!";
        }
    ...
    public String removeCanalInstance(Model model, CanalInstance param) {
        ...
        try {
            ...
            LOG_ERROR.error("removeCanalInstance failed: {}", ExceptionUtils.getFullStackTrace(e));
            errMsg = "删除失败!";
        }
    ...
    @RequestMapping(value = WagonConstants.URL_REMOVE_CANAL_SERVER, method = RequestMethod.GET)
    public String removeCanalServer(Model model, CanalServer param) {
        if (param == null) {
            param = new CanalServer();
        }
        String errMsg = null;

        try {
            canalServerService.deleteById(param.getId());
        } catch (Throwable e) {
            LOG_ERROR.error("removeCanalServer failed: {}", ExceptionUtils.getFullStackTrace(e));
            errMsg = "删除失败!";
        }
    ...
    @RequestMapping(value = WagonConstants.URL_REMOVE_CANAL_SERVICE, method = RequestMethod.GET)
    public String removeCanalService(Model model, CanalService param) {
        String errMsg = null;
        param = param != null ? param : new CanalService();

        List<CanalInstance> canalInstances = canalInstanceService.findByServiceName(param.getServiceName());
        List<CanalServer> canalServers = canalServerService.findByServiceName(param.getServiceName());
        if (!CollectionUtils.isEmpty(canalInstances)) {
            errMsg = "还有实例,不能删除该服务!";
        } else if (!CollectionUtils.isEmpty(canalServers)) {
            errMsg = "还有服务器,不能删除该服务!";
        } else {
            try {
                canalServiceService.deleteById(param.getId());
            } catch (Throwable e) {
                LOG_ERROR.error("removeCanalService failed: {}", ExceptionUtils.getFullStackTrace(e));
                errMsg = "删除失败!";
            }
        }
