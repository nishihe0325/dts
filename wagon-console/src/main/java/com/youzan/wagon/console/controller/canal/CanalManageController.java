...
public class CanalManageController {
    ...
    public String getCanalInstanceDatas(String serviceName) {
        try {
            ...
            for (CanalInstance instance : instances) {
                if (CanalInstance.ServiceState.STOP.getState().equals(instance.getServiceState())) {
                    continue; // 已经停止的不发送
                }
    ...
    @RequestMapping(value = WagonConstants.URL_REGISTER_SERVICE, method = RequestMethod.POST)
    @ResponseBody
    public String registerOrUpdateServiceInfo(@RequestBody String json) {
        LOG_HEARTBEAT.info("receive registerOrUpdateServiceInfo request:{}", json);

        RegisterCanalServiceResponseData responseData = new RegisterCanalServiceResponseData();
        try {
            RegisterCanalServiceRequestData requestData = CanalInstanceData.fromJson(json, RegisterCanalServiceRequestData.class);
            LOG.info("registerOrUpdateServiceInfo: " + requestData.toString());
        } catch (Exception e) {
            LOG.error("handle registerOrUpdateServiceInfo failed, request data:{},\n{}", json, ExceptionUtils.getFullStackTrace(e));
            responseData.setResultCode(RegisterCanalServiceResponseData.RESULT_CODE_FAILED);
            responseData.setResultMsg("shiva handle registerInstance failed.");
        }

        String result = responseData.toJson();
        LOG_HEARTBEAT.info("response registerOrUpdateServiceInfo result:{}", result);
        return result;
    }
